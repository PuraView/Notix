...
actor NotificationService {
    ...
    func cancelNotifications(for terminID: UUID) async {
        ...
        let ids = ["\(base)_day", "\(base)_hour", "\(base)_custom"]
    ...
    // Yeni: öğeye özel tek hatırlatma (ör. X dakika önce)
    func scheduleCustomReminder(for item: TerminItem) async {
        guard let reminder = item.reminder else { return }
        let center = UNUserNotificationCenter.current()
        let base = "termin_\(item.id.uuidString)"
        let triggerDate: Date?

        switch reminder {
        case .minutesBefore(let mins):
            triggerDate = Calendar.current.date(byAdding: .minute, value: -mins, to: item.dateTime)
        }
        ...
        let trigger = UNCalendarNotificationTrigger(dateMatching: Self.components(from: fireDate), repeats: false)
        let request = UNNotificationRequest(identifier: "\(base)_custom", content: content, trigger: trigger)
        do { try await center.add(request) } catch {}
    }
